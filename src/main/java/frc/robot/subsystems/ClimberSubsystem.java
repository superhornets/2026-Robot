...
public class ClimberSubsystem extends SubsystemBase {
  ...
  public ClimberSubsystem() {
    motor = new SparkMax(Constants.CanIds.ClimberMotor1, MotorType.kBrushless);
  ...
  public void periodic() {
    ...
    // Retrieve the control input from the Dashboard
    if (SmartDashboard.getBoolean("Climber Control Mode", false)) {
      /*
       * Get the target velocity from SmartDashboard and set it as the setpoint
       * for the closed loop controller.
       */
      double targetVelocity = SmartDashboard.getNumber("Climber Target Velocity", 0);
    ...
    SmartDashboard.putNumber("Climber Actual Position", encoder.getPosition());
    SmartDashboard.putNumber("Climber Actual Velocity", encoder.getVelocity());

    if (SmartDashboard.getBoolean("Climber Reset Encoder", false)) {
      SmartDashboard.putBoolean("Climber Reset Encoder", false);
